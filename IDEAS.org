...
See also [[https://alex.corcoles.net/notes/tech/internet-basics][Internet basics]].
...
- Test hardware for Linux compatibility
- Test Linux distributions for service compatibility
- Track and improve accessibility
* Make it easier to share NoScript configurations over multiple devices

- Fix underlying issues?
- Configuration merge tool?

* Archive transit data

- Many transport agencies (trains, bus, etc.) publish real-time data in standard formats
- However, you cannot consult historical data
- Make polling, archiving, and publishing historical data easy
- You can use the data to provide "probabilistic" scheduling (at which time should I be at X station to arrive to Y station before HH:MM?)
...
- [[https://mastodon.social/users/pid_eins/statuses/117307883706564416][systemd moving forward on this]]
** Nice mailing lists
...
See also https://alex.corcoles.net/notes/tech/real-time-relations
...
* DeltaChat + webdxc bot framework to control services without exposing them to the Internet
* Code review tool
...
* Rclone-style browser extension

So that webapps can store data in a backend of your choosing.

* Pen and tablet diagram tool

- Recognizes diagram elements and converts them to structured objects.
* Jerarco
...
* [[https://alex.corcoles.net/notes/tech/a-monitoring-system][Monitoring system]]
...
* An HTML-supporting terminal

See https://infosec.exchange/@david_chisnall/116526545563279595

- The terminal can detect programs that output HTML and show the HTML instead of regular console output
- Programs can also signal that they run a web server, and the terminal shows the content of the embedded web server.
  (So an HTML top could provide a real-time user interface.)
- A command could also provide the way to open a URL in the terminal, so the terminal could replace a browser.
...
https://liliputing.com/reterminal-sticky-is-a-50-e-ink-gadget-with-flashable-firmware-that-can-be-a-refrigerator-sign-or-a-pocket-sized-ereade/
https://www.seeedstudio.com/sticky/playground/
...
- Not so important because you can just create your own start page easily, like https://alex.corcoles.net/gemini / gemini://alex.corcoles.net/gemini
